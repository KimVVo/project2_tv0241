...
class MyServer(BaseHTTPRequestHandler):
    ...
    def get_all_keys(self):
        ...
        for row in rows:
            keys.append({
                "kid": row[0],
                "key": row[1],  # You may want to encode this further if needed
                "exp": row[2]
            })
        return keys
...
def init_db():
    ...
    # Insert a key for demonstration
    insert_key(conn, pem, int((datetime.now() + timedelta(days=1)).timestamp()))
    insert_key(conn, expired_pem, int((datetime.now() - timedelta(days=1)).timestamp()))
